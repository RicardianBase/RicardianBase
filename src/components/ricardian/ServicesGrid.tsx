...
const services = [
  {
    ...
    price: "$320",
    priceSuffix: ".00",
    desc: "Milestone-based escrow for guaranteed payments",
    backTitle: "How Smart Escrow Works",
    backDetails: [
      "Company deposits USDC/PYUSD into the smart contract's escrow upon contract creation.",
      "Funds are visible to both parties but untouchable until milestones are approved.",
      "When a milestone is approved, payment releases instantly — no invoices, no AP department.",
      "Configurable timeout enforcement: if review isn't completed in time, auto-release kicks in.",
    ],
  },
  {
    image: service2,
    ...
    backTitle: "The Ricardian Magic",
    backDetails: [
      "System simultaneously generates a human-readable legal contract (PDF) and a matching Base smart contract.",
      "A cryptographic hash permanently links the two — creating tamper-proof bond.",
      "Both parties sign with electronic signature (ESIGN Act) and wallet signature (Base blockchain).",
      "Any post-signing modification to the legal document is instantly detectable via hash mismatch.",
    ],
  },
  {
    image: service3,
    ...
    price: "$5,000",
    priceSuffix: ".00",
    desc: "Click approve → instant stablecoin settlement",
    backTitle: "Auto-Pay Workflow",
    backDetails: [
      "Contractor submits deliverables through the platform for each milestone.",
      "Company reviews and clicks 'Approve' — smart contract instantly releases payment to contractor's wallet.",
      "Payment in USDC or PYUSD — always worth $1 USD, zero crypto volatility.",
  ...
  },
  {
    ...
    backTitle: "Compliance Built In",
    backDetails: [
      "Every user (companies and contractors) must verify identity before creating or signing contracts.",
      "Integrated third-party KYC/AML provider for global identity checks.",
      "SOC 2 Type II certification-ready architecture from day one.",
      "Compliant with GENIUS Act federal stablecoin framework and relevant data protection laws.",
    ],
  },
];
...
const FlipCard = ({ service, index, isInView }: { service: typeof services[0]; index: number; isInView: boolean }) => {
  ...
  return (
    <div
      className={`min-w-[300px] md:min-w-[340px] flex-shrink-0 ${isInView ? "animate-fade-in-up" : ""}`}
  ...
  return (
    <section ref={ref} className="bg-background py-20 md:py-28 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div
          className={`flex items-center justify-center gap-2 text-xs text-muted-foreground mb-6 ${isInView ? "animate-fade-in-up" : ""}`}
        ...
        >
          {services.map((s, i) => (
            <FlipCard key={i} service={s} index={i} isInView={isInView} />
          ))}
        </div>
      </div>
    </section>
